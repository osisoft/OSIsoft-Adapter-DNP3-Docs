...
If a startup integrity scan is configured, it might be used for discovery. If no startup integrity scan is configured, but an integrity scan at a set interval is defined, then the first integrity scan still performs on startup, but it is not used for discovery. If a startup integrity scan is configured, it runs whenever the adapter starts or whenever a shutdown is indicated by the outstation's IIN bits. 
